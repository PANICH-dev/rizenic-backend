...
app.get('/api/quotas', async (req, res) => {
  try { 
    const result = await pool.query('SELECT * FROM rizenic_quotas ORDER BY quota_type ASC, quota_date DESC');
...
app.get('/api/car-models', async (req, res) => {
  try { 
    const result = await pool.query('SELECT model_id, car_brand, car_model FROM rizeniccarmodelmaster ORDER BY car_brand ASC');
    res.json(result.rows); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.post('/api/car-models', async (req, res) => {
  try { 
    await pool.query('INSERT INTO rizeniccarmodelmaster (car_brand, car_model) VALUES ($1, $2)', [req.body.car_brand, req.body.car_model]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.put('/api/car-models/:id', async (req, res) => {
  try { 
    await pool.query('UPDATE rizeniccarmodelmaster SET car_brand=$1, car_model=$2 WHERE model_id=$3', [req.body.car_brand, req.body.car_model, req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.delete('/api/car-models/:id', async (req, res) => {
  try { 
    await pool.query('DELETE FROM rizeniccarmodelmaster WHERE model_id = $1', [req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.get('/api/insurances', async (req, res) => {
  try { 
    const result = await pool.query('SELECT insurance_code, insurance_name, insurance_type FROM rizenicinsurancemaster ORDER BY insurance_code ASC');
    res.json(result.rows); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.post('/api/insurances', async (req, res) => {
  try { 
    await pool.query('INSERT INTO rizenicinsurancemaster (insurance_code, insurance_name, insurance_type) VALUES ($1, $2, $3)', [req.body.insurance_code, req.body.insurance_name, req.body.insurance_type]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.put('/api/insurances/:id', async (req, res) => {
  try { 
    await pool.query('UPDATE rizenicinsurancemaster SET insurance_name=$1, insurance_type=$2 WHERE insurance_code=$3', [req.body.insurance_name, req.body.insurance_type, req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.delete('/api/insurances/:id', async (req, res) => {
  try { 
    await pool.query('DELETE FROM rizenicinsurancemaster WHERE insurance_code = $1', [req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.put('/api/customer-types/:id', async (req, res) => {
  try { 
    await pool.query('UPDATE rizeniccustomertypemaster SET type_name = $1 WHERE customer_type_id = $2', [req.body.type_name, req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.get('/api/body-parts', async (req, res) => {
  try { 
    const result = await pool.query('SELECT * FROM rizenic_body_parts ORDER BY id ASC');
    res.json(result.rows); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.post('/api/body-parts', async (req, res) => {
  try { 
    await pool.query('INSERT INTO rizenic_body_parts (category, part_name) VALUES ($1, $2)', [req.body.category, req.body.part_name]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.put('/api/body-parts/:id', async (req, res) => {
  try { 
    await pool.query('UPDATE rizenic_body_parts SET category=$1, part_name=$2 WHERE id=$3', [req.body.category, req.body.part_name, req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.delete('/api/body-parts/:id', async (req, res) => {
  try { 
    await pool.query('DELETE FROM rizenic_body_parts WHERE id=$1', [req.params.id]); 
    res.json({ success: true }); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.post('/api/report', async (req, res) => {
  try {
    const {
      ...
      target_finish_date, actual_finish_date, repair_finish_date, delivery_date,
      contact_date, arrived_date, appointment_date, car_plate,
    ...
    const values = [
      ...
      notes || null, job_status || null, target_finish_date || null, actual_finish_date || null, repair_finish_date || null, delivery_date || null,
      contact_date || null, arrived_date || null, appointment_date || null, car_plate || null,
  ...
  try {
    ...
    const queryText = `
      UPDATE rizenicreport SET 
        ...
        target_finish_date=$23, actual_finish_date=$24, repair_finish_date=$25, delivery_date=$26,
        contact_date=$27, arrived_date=$28, appointment_date=$29, car_plate=$30,
        station_kho=$31, station_pou=$32, station_puan=$33, station_pon=$34, station_prak=$35, station_kat=$36,
        station_qc=$37, station_mag=$38, station_kraj=$39, station_film=$40, station_pak=$41, station_ready=$42,
        repair_notes=$43,
        epc_no=$44, part_status=$45, order_part_date=$46, est_part_date=$47, ordered_part_names=$48,
        department_routing=$49,
        is_parked=$50
      WHERE id=$51;
    `;
    ...
    const values = [
      ...
      target_finish_date || null, actual_finish_date || null, repair_finish_date || null, delivery_date || null,
      contact_date || null, arrived_date || null, appointment_date || null, car_plate || null,
...
// 🌟 API อัปเดตข้อมูลแบบด่วน (Fast Update) ครอบคลุมทุกฟิลด์
app.put('/api/report/:id/fast-date', async (req, res) => {
  try {
    const { field, value } = req.body;
    
    const validFields = [
      'target_finish_date', 'repair_finish_date', 'actual_finish_date', 'delivery_date', 'contact_date', 'appointment_date', 'arrived_date', 'order_part_date', 'est_part_date', 'expected_finish_date',
      ...
      'customer_phone', 'quotation_no', 'job_order_no'
    ];
    if (!validFields.includes(field)) return res.status(400).json({ error: 'ไม่อนุญาตให้แก้ฟิลด์นี้' });
    
    let safeValue = (value === '' || value === undefined) ? null : value;
...
app.get('/api/part-orders', async (req, res) => {
  try { 
    const result = await pool.query('SELECT * FROM rizenic_part_orders ORDER BY order_id DESC');
    res.json(result.rows); 
  } catch (e) { 
    res.status(500).json({ error: e.message }); 
  }
...
app.get('/api/user-preferences/:empName', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT hidden_columns FROM user_column_preferences WHERE emp_name = $1', [empName]);
    res.json({ hidden_columns: result.rows.length > 0 ? result.rows[0].hidden_columns : null });
  } catch (error) { 
...
app.post('/api/user-preferences', async (req, res) => {
  try {
    ...
    const queryText = `
      ...
      ON CONFLICT (emp_name) DO UPDATE SET hidden_columns = EXCLUDED.hidden_columns, updated_at = CURRENT_TIMESTAMP;
    `;
    await pool.query(queryText, [emp_name, JSON.stringify(hidden_columns)]);
    res.json({ success: true });
  } catch (error) { 
    res.status(500).json({ error: error.message }); 
  }
