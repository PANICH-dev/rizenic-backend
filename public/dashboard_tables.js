...
function renderSASection() {
    ...
    const activeJobs = (window.filteredJobs || []).filter(job => {
...
function openSAModal(saName) {
    ...
    const jobsToShow = (window.filteredJobs || []).filter(job => {
        const st = (job.job_status || '').trim();
...
function renderStationTable() {
    ...
    // 🎯 กรองเอาเฉพาะรถที่จอดในศูนย์
    const inRepairCars = (window.filteredJobs || []).filter(j => {
        ...
        return isParked && typeof computeHighestStationIFS === 'function' && activeStations.includes(computeHighestStationIFS(j));
    });
    ...
    tbody.innerHTML = inRepairCars.map(j => {
        ...
        const station = typeof computeHighestStationIFS === 'function' ? computeHighestStationIFS(j) : '-';
...
function renderParkedCars() {
    ...
    const parkedCars = (window.filteredJobs || []).filter(j => parkedStatuses.some(p => (j.job_status || '').includes(p)));
    ...
    tbody.innerHTML = parkedCars.map(j => {
        ...
        return `
            <tr class="cursor-pointer hover:bg-amber-50/80 transition-colors border-b border-slate-100" onclick="sessionStorage.setItem('edit_job_id', '${j.id}'); window.location.href='index.html';">
                ...
                <td class="font-bold text-orange-600 text-xs px-4 py-3"><i class="fa-solid fa-wrench"></i> ${typeof computeHighestStationIFS === 'function' ? computeHighestStationIFS(j).replace(/[0-9.]/g, '') : '-'}</td>
...
// ==========================================
// 🗓️ ปฏิทินปฏิบัติงาน (ดีไซน์ Original สุดคลีนจากรูป ds3_2.jpg)
// ==========================================
function renderCalendarByRange(startStr, endStr) {
    ...
    while (current <= endCalendar) {
        ...
        if (isOutOfRange) {
            ...
            const arrivedJobs = (window.filteredJobs || []).filter(j => cleanDate(j.arrived_date) === dateStr || cleanDate(j.appointment_date) === dateStr);
            const targetJobs = (window.filteredJobs || []).filter(j => cleanDate(j.target_finish_date) === dateStr);
            const deliveredJobs = (window.filteredJobs || []).filter(j => cleanDate(j.delivery_date) === dateStr);
            ...
            <div class="${cellClass}" onclick="openCalendarModal('${dateStr}')">
                ...
                <div class="flex-1 flex justify-center items-end gap-3 pb-4">
                    <!-- เข้าจอด (น้ำเงิน) -->
                    <div class="flex flex-col items-center justify-end h-[65px] w-3 relative group">
...
function renderJobTableInModalGroupedBySA(jobs) {
    ...
    Object.keys(groupedBySA).sort().forEach((sa, index) => {
        ...
        groupedBySA[sa].forEach(j => {
            ...
            // 🎯 ปุ่มดูข้อมูลใน Modal แก้ไขแล้ว
            finalHtml += `
