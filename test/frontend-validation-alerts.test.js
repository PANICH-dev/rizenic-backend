...
test('pages cache-bust the validation-alert JS revisions so browsers do not keep stale save logic', () => {
  ...
  assert.match(jobs, /jobs\.js\?v=2\.5/);
  assert.match(jobsTable, /jobs_table_core\.js\?v=23\.3/);
  assert.match(jobsTable, /jobs_table_modals\.js\?v=23\.1/);
  assert.match(parts, /parts_ui\.js\?v=23\.5/);
  assert.match(repair, /repair\.js\?v=7/);
  assert.match(index, /sa_core\.js\?v=19\.2/);
  assert.match(index, /sa_parts\.js\?v=19\.1/);
  assert.match(portal, /sa_core\.js\?v=19\.2/);
